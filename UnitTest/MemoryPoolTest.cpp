...
namespace UnitTest
{
    ...
    public:
        ...
        TEST_METHOD(AllocPattern1)
        {
            MemoryPool pool;
            List<unsigned char> buffer;
            buffer.SetSize(1 << 21);
            pool.Init(buffer.Buffer(), 6, 1<<15);
            auto ptr0 = pool.Alloc(128);
            auto ptr1 = pool.Alloc(128);
            auto ptr2 = pool.Alloc(384);
            auto ptr3 = pool.Alloc(384);
            pool.Free(ptr0, 128);
            pool.Free(ptr2, 384);
            pool.Free(ptr1, 128);
            pool.Free(ptr3, 384);
