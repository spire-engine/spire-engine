        ...
        struct mdiaBox
        {
            ...
            struct minfBox
            {
                ...
                struct stblBox
                {
                    ...
                    uint32_t updateSize()
                    {
                        auto rs = 8 + stsd.updateSize() + ToBigEndian32(stsc.size) + ToBigEndian32(stts.size) +
                                  ToBigEndian32(stco.size) + stsz.updateSize() + stss.updateSize();
                        size = ToBigEndian32(rs);
                        return rs;
                    }
                    void Write(Stream *s)
...
public:
    ...
    void WriteFrame(SFrameBSInfo &info)
    {
        ...
        if (info.eFrameType == videoFrameTypeIDR || info.eFrameType == videoFrameTypeI)
        {
            ...
            moov.track.mdia.minf.stbl.stss.numEntries =
                ToBigEndian32((uint32_t)moov.track.mdia.minf.stbl.stss.entries.Count());
        }
    }
    ...
    virtual void Init(VideoEncodingOptions options, Stream *outputStream) override
    {
        ...
        const unsigned char mp4header[] = {0x00, 0x00, 0x00, 0x20, 0x66, 0x74, 0x79, 0x70, 0x69, 0x73, 0x6F, 0x6D, 0x00,
            0x00, 0x02, 0x00, 0x69, 0x73, 0x6F, 0x6D, 0x69, 0x73, 0x6F, 0x32, 0x61, 0x76, 0x63, 0x31, 0x6D, 0x70, 0x34,
            0x31, 0x00, 0x00, 0x00, 0x08, 0x66, 0x72, 0x65, 0x65, 0x00, 0x00, 0x00, 0x00, 0x6D, 0x64, 0x61, 0x74};
